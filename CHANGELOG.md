...
- **Watch** `buildMetadata()` (`src/lib/seo.ts`) never appends the brand itself - only the root layout's `title.template` does, once, for every page. Any new page's `title`/`seoTitle` string should read as the page's own headline, not include "Niscala Furniture" - the template adds it automatically. This is easy to reintroduce by copy-pasting a title from marketing copy that already has the brand name in it.
